...
##Q1.1 It seems there is a relationship but because we are throwing species and sex all into the same figure, we can't be sure what the relationship could be for each of those variables.
...
ggplot(data = penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point(aes(color = species,
                 shape = species)) +
  scale_color_brewer(palette = "Dark2") +
  scale_shape_manual(values = c(1, 11, 14))
```
...
## Q2.1We decided to make our figure for a policy maker with the intentions of making sure there is legislation in place to limit the take of each of these abalone species and to keep legislation active if its already in place.
```{r}
ggplot(data = ab, mapping = aes(x = Year, y = Abalone_Landings_lbs)) +
...
##Q2.3 We are changing it to a line graph in addition to increasing the line thickness and changing the color to the name of the species.
...
ggplot(data = ab, mapping = aes(x = Year, y = Abalone_Landings_lbs)) +
  ...
  scale_color_manual(values=c("Black"="black","Green"="green","Pink"="pink","Red"="red","While"="blue"))
  facet_wrap(Abalone_Species ~ .)
```


### Q2.5: Explain your graph
##Q2.5 We decided that for a policymaker, it would be easier for them to see the colors matching up with species name, other than white (you can't see it) and we combined it onto one graph as policymakers would most likely be annoyed to see 5 when it could have been one.
